...
完整 API 和错误行为见 [docs/API.md](docs/API.md)。
JSON 快照 v1 的正式契约见 [docs/run-snapshot-v1.schema.json](docs/run-snapshot-v1.schema.json)。
...
- `moon check` 通过。
- `moon test` 通过，当前共 29 项测试：28 项核心行为测试和 1 项 README 可执行示例。
- 测试覆盖 DAG、状态迁移、安全快照、过期计划、未知 trace 任务、JSON 合法性和三种导出。
